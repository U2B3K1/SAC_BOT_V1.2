...
# =============================================
@router.get("/users")
async def list_users(current_user: SuperUser):
    result = db.table("users").select("*").order("created_at").execute()
    return result.data


@router.post("/users", status_code=201)
async def create_user(body: UserCreate, current_user: SuperUser):
    ...
    await log_audit(current_user["id"], "users", result.data[0]["id"], "INSERT", None, result.data[0])
    return result.data[0]


@router.patch("/users/{user_id}")
async def update_user(user_id: str, body: UserUpdate, current_user: SuperUser):
    ...
    await log_audit(current_user["id"], "users", user_id, "UPDATE", old.data, result.data[0])
    return result.data[0]


@router.delete("/users/{user_id}")
async def deactivate_user(user_id: str, current_user: SuperUser):
...
async def list_departments(current_user: CurrentUser):
    result = db.table("departments").select("*").order("sort_order").execute()
    return result.data


@router.post("/departments", status_code=201)
async def create_department(body: DepartmentCreate, current_user: SuperUser):
    result = db.table("departments").insert(body.model_dump()).execute()
    return result.data[0]


@router.patch("/departments/{dept_id}")
async def update_department(dept_id: str, body: DepartmentUpdate, current_user: SuperUser):
    updates = {k: v for k, v in body.model_dump().items() if v is not None}
...
# =============================================
@router.get("/products")
async def list_products(current_user: CurrentUser, department_id: str = None):
...
async def create_product(body: ProductCreate, current_user: SuperUser):
    result = db.table("products").insert(body.model_dump()).execute()
    await log_audit(current_user["id"], "products", result.data[0]["id"], "INSERT", None, result.data[0])
    return result.data[0]


@router.patch("/products/{product_id}")
async def update_product(product_id: str, body: ProductUpdate, current_user: SuperUser):
    ...
    await log_audit(current_user["id"], "products", product_id, "UPDATE", old.data, result.data[0])
    return result.data[0]


@router.delete("/products/{product_id}")
async def deactivate_product(product_id: str, current_user: SuperUser):
...
async def list_ingredients(current_user: CurrentUser):
    result = db.table("ingredients").select("*").eq("is_active", True).order("name").execute()
    return result.data


@router.post("/ingredients", status_code=201)
async def create_ingredient(body: IngredientCreate, current_user: SuperUser):
    result = db.table("ingredients").insert(body.model_dump()).execute()
    return result.data[0]


@router.patch("/ingredients/{ingredient_id}")
async def update_ingredient(ingredient_id: str, body: IngredientUpdate, current_user: SuperUser):
    updates = {k: v for k, v in body.model_dump().items() if v is not None}
...
# =============================================
@router.get("/recipes")
async def list_recipes(current_user: CurrentUser):
...
async def create_recipe(body: RecipeCreate, current_user: SuperUser):
    # Recipe asosiy
    recipe_data = {
...
async def update_recipe(recipe_id: str, body: RecipeUpdate, current_user: SuperUser):
...
async def list_expense_categories(current_user: CurrentUser):
...
async def create_expense_category(body: ExpenseCategoryCreate, current_user: SuperUser):
